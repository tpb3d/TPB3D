...
*************************************************************************/
bool FRCSWindow::Create (CEGUI::Window* pRoot)
{
...
// Code provided by Paul D Turner (C) 2005
   ...
   mpWnd = (FrameWindow*)winMgr.createWindow( LOOK "/FrameWindow", "FRCS");
   ...
   // is that these FRCS are aways relative to the display rather than the
   // parent window.
   //
   ...
   CEGUI::Checkbox* pBox = static_cast<CEGUI::Checkbox*>(winMgr.createWindow(LOOK "/Checkbox", "FRCS/SoundChechBox"));
   ...
   mpWnd->subscribeEvent (FrameWindow::EventCloseClicked, Event::Subscriber(&FRCSWindow::OnClose, this));

   // subscribe handler that quits the application
   mpWnd->subscribeEvent (FrameWindow::EventCloseClicked, Event::Subscriber(&FRCSWindow::OnSoundCheck, this));
...
bool FRCSWindow::OnClose(const CEGUI::EventArgs& e)
{
   Destroy();
   return true;
}

bool FRCSWindow::OnSoundCheck(const CEGUI::EventArgs& e)
{
   return true;
}
