...
class PngImage{
	...
	PngImageDot dot4;
	void Init(){	pd.Data=NULL; 	pd.Palette=NULL;	}
	PngImage(){	Init();	}
	PngImage(char*file){	Init();		Load(file);	}
	bool Load(char*file){	return(pngLoadRaw(file,&pd));	}
	int ImgIndex(int x,int y){	int ii=y*pd.Width+x;	int i=pd.Components*ii;	return(i);	}
	void*imgptr(int x,int y){	return((void*)&pd.Data[ImgIndex(x,y)]);	}
	//color: reference palette entry as address
	PngImageDot&
	color(int entry){	PngImageDot*p=(PngImageDot*)pd.Palette;	return(p[entry]);	}
