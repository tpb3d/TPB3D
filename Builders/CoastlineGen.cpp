...
RadialCoastInDegrees(int nRefinements,float angleStep,float avgRadius,float dv){
   ...
   int npts = int(2.0*pi/angleStepRadians)+1;
   Init0(npts,dv);
   //std::cout<<"Radial Island Maker: ("<<angleStepRadians<<")\r\n";

   for(int i=0,a=0;i<npts-1;i++,a+=angleStepRadians)
   {
      r=frandCenter(avgRadius,avgRadius*dv/2);
      points[i].x=r*cos((float)a);
      points[i].y=r*sin((float)a);
...
float IslandCoastline::GetEdge(IslandCoastline::edge e)
{
   float ge=0.f;
   float largeVal=1.0E+07f;
   switch(e)
   {
   case(IslandCoastline::redge):
   case(IslandCoastline::dedge):
      ge=largeVal;
      break;
   case(IslandCoastline::ledge):
   case(IslandCoastline::uedge):
      ge=-largeVal;
      break;
   default:
      return(ge);
...
bool IslandCoastline::IsPointOnIsland(Vector2f p)
{
   const float step=0.5f;
   bool oic;
   float x=0.f;
   float l=this->GetEdge(IslandCoastline::ledge);
   float y=0.f;
