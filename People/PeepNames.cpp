...
void
NameEntry::
	Init(char*n,long w){	name=n;weight=w;	}
NameEntry::
	NameEntry(char*n,long w){	Init(n,w);	}
void
NameEntry::
	Init(char*n,long w,Name2Features a1,Name2Features a2,Name2Features a3,Name2Features a4){
...
NameEntry::
	NameEntry(char*n,long w,Name2Features a,Name2Features b,Name2Features c){
		Name2Features d(0,0,false);		Init(n,w,a,b,d,d);
		}
NameEntry::
...
NameEntry::
	NameEntry(char*n,long w,Name2Features a){
		Name2Features d(0,0,false);		Init(n,w,a,d,d,d);
		}
NameEntry::
	NameEntry(char*n,long w,Name2Features a[]){
...
void
NameGenerator::
	Init(short slocation){	Init("",slocation);	}
NameGenerator::
	NameGenerator(short slocation)	{	Init(slocation);	}
void
NameGenerator::
	Destroy()	{
		int rv;
...
NameGenerator::
	NameGenerator(char*file,short slocation)	{	Init(file,slocation);	}
NameGenerator::
	NameGenerator()	{	Init("",locIndexWorld);	}
void
NameGenerator::
	ReadListFromFile(char*file)	{
...
void
NameGenerator::
	Init(char*file,short slocation)	{
		location=slocation;
		list=NULL;	numEntries=0;
		if(file[0]==0)	/*""*/	{				}
		else		ReadListFromFile(file);
		}
void
NameGenerator::
	GrowList(){	GrowList(1);	}
void
NameGenerator::
	GrowList(short nnewentries){	ResizeList(numEntries+nnewentries);	}
void
NameGenerator::
	ResizeList(short snentries)	{
		if(snentries==0)
			list=(NameEntry*)realloc((void*)list,(long)sizeof(NameEntry)*(1));
		else
			list=(NameEntry*)realloc((void*)list,(long)sizeof(NameEntry)*(snentries));
		numEntries=snentries;
		}
void
NameGenerator::
	Add2List(NameEntry n){
		GrowList();
		list[numEntries-1]=n;
		}
void
NameGenerator::
	Add2List(char*name,long weight)	{
		GrowList();
		list[numEntries-1].weight=weight;			list[numEntries-1].name=name;
		}
void
NameGenerator::
	CleanList(){
...
long
NameGenerator::
	InRange(long n,long l,long h)  {   return((n>=l)&&(n<=h)); }
NameString
NameGenerator::
	GetListName(){
		NameEntry errorEntry("(ERROR)",-1);
		long lrv=rand()%GetTotNameWeight();		long lo,hi;
		for(int i=0,lo=0,hi=-1;i<numEntries;i++){
			lo=hi+1;			hi=lo+list[i].weight;
			if(InRange(lrv,lo,hi)){	return(list[i].name);	}
			}
...
void
NameGenerator::
	LessThanSwap(short i,short j){
		if(list[i].weight<list[j].weight)	SwapEntries(i,j);
		}
void
NameGenerator::
	SwapEntries(short i,short j){
		NameEntry tmp("",0);
...
NameGenerator::
	OptimizeList(){
		...
		for(int i=0;i<numEntries-1;i++){	for(int j=i+1;j<numEntries;j++){
			LessThanSwap(i,j);
			}	}//for j and i
		}//OptimizeList()
...
NameGenFileParser::
	NameGenFileParser(char*file,NameGenerator&sng){
		f=fopen(file,"r+");	this->ng=&sng;
		}
NameGenFileParser::
	~NameGenFileParser(){
		if(f)	fclose(f);	this->ng=NULL;
		}
bool
NameGenFileParser::
	ReadLine(){
...
NameGenFileParser::
	ReadEntry(){
		if(f){
			ReadNextUsedLine();
			if(Eof())return false;
			char name[64];	short weight;	char aprnames[4][32]; short aprtypes[4];	short aprwts[4];	int nscans;	bool aprused[4];
			...
			for(int i=0;i<4;i++){	aprtypes[i]=(short)ConvTypeString(aprnames[i]);	aprused[i]=aprwts[i]!=0;	}
			if(nscans>=10)
				ng->Add2List(NameEntry(name,weight,Name2Features(aprwts[0],aprtypes[0],aprused[0]),Name2Features(aprwts[1],aprtypes[1],aprused[1]),Name2Features(aprwts[2],aprtypes[2],aprused[2]),Name2Features(aprwts[3],aprtypes[3],aprused[3])));
			else if(nscans>=8)
				ng->Add2List(NameEntry(name,weight,Name2Features(aprwts[0],aprtypes[0],aprused[0]),Name2Features(aprwts[1],aprtypes[1],aprused[1]),Name2Features(aprwts[2],aprtypes[2],aprused[2])));
			else if(nscans>=6)
				ng->Add2List(NameEntry(name,weight,Name2Features(aprwts[0],aprtypes[0],aprused[0]),Name2Features(aprwts[1],aprtypes[1],aprused[1])));
			else if(nscans>=4)
				ng->Add2List(NameEntry(name,weight,Name2Features(aprwts[0],aprtypes[0],aprused[0])));
			else if(nscans>=2)
				ng->Add2List(NameEntry(name,weight));
			else 	{
				return(false);
				}
			return(true);
...
humanFeatureTypes
NameGenFileParser::
	ConvTypeString(char*s){
		humanFeatureTypes rv=hfUnk;
		struct{	char name[16];	humanFeatureTypes ftype;	}
		...
		for(int i=0;i<nhumanFeatureTypes;i++){	if(strcmp(typetable[i].name,s)==0)	rv=(humanFeatureTypes)i;	}
		return(rv);
		}//NameGenFileParser::ConvTypeString()
bool
NameGenFileParser::
	Eof(){	bool rv=feof(f);	return(rv);	}
void
NameGenFileParser::
	ReadNextUsedLine()	{
		bool done=false;
